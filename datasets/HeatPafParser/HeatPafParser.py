...
class HeatPafParser:
    ...
    def forward(self, heatmap, pafmap):
        ...
        if self.req[0] == req.add:
            out_temp = self.F.ones_like(out) * -1
            ...
            mobula.func.heat_paf_parser(heatmap, pafmap, self.limb_sequence, number_of_parts, len(self.limb_sequence), w0, h0, self.max_number_person, out_temp)
            self.y[:] += out_temp
        else:
            self.y[:] = -1
            mobula.func.heat_paf_parser(heatmap, pafmap, self.limb_sequence, number_of_parts, len(self.limb_sequence), w0, h0, self.max_number_person, self.y)
    ...
    def infer_shape(self, in_shape):
        ...
        assert h0 == h1
        assert w0 == w1
        return in_shape, [(self.max_number_person, number_of_parts, 2)]  # x, y
