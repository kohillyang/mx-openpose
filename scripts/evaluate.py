...
if __name__ == '__main__':
    ...
    for i in tqdm.tqdm(range(len(imgIds))):
        ...
        pafmaps = [mx.nd.contrib.BilinearResize2D(x[-2], width=image_ori.shape[1], height=image_ori.shape[0]).asnumpy()
                   for x in net_results]
        heatmap = np.mean(heatmaps, axis=0)[0].transpose((1, 2, 0))
        pafmap = np.mean(pafmaps, axis=0)[0].transpose((1, 2, 0))

        r = parse_heatpaf(image_ori, heatmap, pafmap, None, image_id=image_id, fscale=1.0)
        results.extend(r)
